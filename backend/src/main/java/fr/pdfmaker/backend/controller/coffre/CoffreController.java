...
public class CoffreController {
   ...
   /*@PostMapping(value = "/" , consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
    public ResponseEntity<byte []> accessCoffreFort (@ModelAttribute PdfEncryptRequestDto fichier , @AuthenticationPrincipal Utilisateur utilisateur) {
    ...
    /*
     @PostMapping("/decrypt/{idUser}")
    public ResponseEntity<byte[]> decryptFile(
            @PathVariable Long idUser,
            @RequestParam("file") MultipartFile file) throws Exception {

        byte[] decrypted = encryptCoffreService.decryptPdf(idUser, file.getBytes());
        return ResponseEntity.ok()
            .header("Content-Disposition", "attachment; filename=decrypted.pdf")
            .body(decrypted);
    }*/
